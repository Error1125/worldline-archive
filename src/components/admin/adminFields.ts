...
export const RECORD_TYPES: RecordTypeDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "status", label: "档案状态（可选）", kind: "select", options: [{ value: "favorite", label: "favorite · 收藏" }, { value: "rotation", label: "rotation · 轮播" }, { value: "memory", label: "memory · 回忆" }, { value: "archived", label: "archived · 封存" }] },
      { name: "rating", label: "评分（0–10，可选）", kind: "number", min: 0, max: 10, step: 0.5 },
      { name: "thoughts", label: "补充感想（可选）", kind: "textarea", rows: 2 },
      { name: "moods", label: "心情标签（可选）", kind: "tags", placeholder: "回车添加" },
      { name: "appleMusicId", label: "Apple Music ID（可选）", kind: "text" },
      { name: "appleMusicStorefront", label: "Storefront（如 jp / cn / us）", kind: "text" },
      { name: "appleMusicArtworkUrl", label: "Apple Music artwork URL / 站内路径", kind: "text" },
      { name: "appleMusicPreviewUrl", label: "试听 preview URL（可选）", kind: "text" },
      { name: "appleMusicReleaseDate", label: "发行日期（可选）", kind: "date" },
      { name: "appleMusicDurationMs", label: "时长（毫秒，可选）", kind: "number", min: 1, step: 1 },
      { name: "appleMusicGenres", label: "流派（可选）", kind: "tags", placeholder: "回车添加" },
      ...coverFields,
      ...commonTail,
    ],
  },
