...
export default function PlaylistShelf({ playlists, activePlaylistId, transitioningPlaylistId, onSelect }: { playlists: MusicPlaylist[]; activePlaylistId?: string; transitioningPlaylistId?: string; onSelect: (id: string, trigger: HTMLButtonElement) => void }) {
  ...
  return <section className="playlist-shelf-section" aria-labelledby="playlist-shelf-title">
    ...
    <div ref={shelf} className="playlist-shelf" data-transition-active={Boolean(transitioningPlaylistId)} onWheel={e => { if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) { e.preventDefault(); e.currentTarget.scrollLeft += e.deltaY; } }} onPointerDown={e => { if (e.button !== 0) return; drag.current = { x: e.clientX, left: e.currentTarget.scrollLeft, moved: false }; e.currentTarget.setPointerCapture(e.pointerId); }} onPointerMove={e => { if (!e.currentTarget.hasPointerCapture(e.pointerId)) return; const delta = e.clientX - drag.current.x; if (Math.abs(delta) > 8) drag.current.moved = true; e.currentTarget.scrollLeft = drag.current.left - delta; }} onPointerUp={e => releasePointer(e.currentTarget, e.pointerId)} onPointerCancel={e => releasePointer(e.currentTarget, e.pointerId)}>{playlists.map((playlist) => <PlaylistPackageCard key={playlist.id} playlist={playlist} active={playlist.id === activePlaylistId} transitioning={playlist.id === transitioningPlaylistId} onSelect={(trigger) => { if (!drag.current.moved) onSelect(playlist.id, trigger); }} />)}</div>
  </section>;
}
