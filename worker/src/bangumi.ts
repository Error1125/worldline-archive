...
type Config = { username: string; syncScopes: string[]; schedule: "manual" | "6h" | "daily" };
...
export async function loadBangumiConfig(env: GitHubEnvLike): Promise<Config> {
  ...
  return { username: value.username.trim(), syncScopes: value.syncScopes ?? Object.values(status), schedule: value.schedule === "6h" || value.schedule === "daily" ? value.schedule : "manual" };
}
...
export async function runBangumiSync(env: BangumiEnvLike) {
  ...
  for (const item of remote) {
    ...
    const values: Array<[string, string]> = [["status", quote(status[item.type] ?? "planned")], ["externalIds", JSON.stringify({ bangumi: Number(id) })], ["syncedAt", quote(now)]];
    ...
    if (existing) { let next = existing.content; for (const [key, value] of values) if (key !== "syncedAt") next = replaceLine(next, key, value); if (next === existing.content) unchanged++; else { next = replaceLine(next, "syncedAt", quote(now)); changes.push({ path: existing.path, content: next }); updated++; } }
