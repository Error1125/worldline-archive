# Worldline Archive / 个人世界綫存档点

> 一个把生活、番剧、音乐、项目、Bug 和深夜念头混在一起的**个人世界綫存档点**。
> 不是普通博客，不是技术博客，也不是 Dashboard —— ：深夜、星光、毛玻璃、终端、档案馆。
...
> 明确不使用：Next.js / Nuxt、大型 UI 库（AntD/MUI）、shadcn 皮肤、富文本编辑器、three.js / 3D、重型状态库、整站 SPA。站点本体保持纯静态；v5 的鉴权与发布走独立的 Cloudflare Worker（见下节），仓库与前端代码中没有任何秘密。

---

## 🛰️ v5.0：观测台（隐藏控制台）

v5 起，这不只是展示站——通过隐藏的移动优先控制台可以直接发布内容、调站点设置、管理媒体：

- **发布闭环**：登录 → `/admin/publish/*` 填表 → Worker 生成 Markdown 并 commit → Actions 自动部署 → 前台出现新记录 → 首页 Worldline 数值跳变。七类内容（说说/文章/照片/项目/音乐/番剧/Bug）全部可发。
- **安全**：口令登录换 httpOnly 签名 cookie；GitHub token（写权限）只存于 Worker secrets；未登录访问 `/admin/*` 一律弹回登录页；访客导航不出现任何后台入口，登录过的浏览器才会在页脚看到低调的「观测台 / console」。
- **控制台**：总览（Worldline 状态 + 仓库/Actions 部署状态 + 内容统计）、发布、媒体（外链登记 / 可选 R2 直传）、设置（档案 / 站点 / 世界线引擎参数），375px 单手可用。
- **Worldline 引擎**：基线 1.048596，按类型权重 + 14 天指数衰减 + 影响倍率实时计算，参数可在线调整。

部署与使用：**[docs/V5_ADMIN.md](docs/V5_ADMIN.md)**（10 分钟接通）· Worker 端见 **[worker/README.md](worker/README.md)**。
